...
import { type CodebaseAnalysis, HandlerExtractor, TLAGenerator } from "@fairfox/polly-verify";
...
for (const handler of handlersWithVerification) {
  ...
  console.log(
    `   Location: ${path.relative(exampleRoot, handler.location.file)}:${handler.location.line}`
  );
