...
type Message<TType extends string, TPayload, TResponse> =
  TPayload & { type: TType; readonly __response?: TResponse }

export type TodoMessages =
  | Message<'USER_LOGIN', { userId: string; name: string; role: 'user' | 'admin' }, { success: true; user: User }>
  | Message<'USER_LOGOUT', {}, { success: true }>
  | Message<'TODO_ADD', { text: string }, { success: true; todo: Todo }>
  | Message<'TODO_TOGGLE', { id: string }, { success: true; todo: Todo } | { success: false; error: string }>
