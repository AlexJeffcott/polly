...
#### `assertNever` exhaustiveness helper (polly#142)

`@fairfox/polly/guards` gains `assertNever(value: never): never`. Placed in a
`switch` `default` arm over a discriminated union, it turns a forgotten variant
into a compile-time error — `value` is only assignable to `never` once every
case is handled — and throws at runtime as a backstop for values that reach the
arm from outside the type system.

#### WCAG contrast helpers for theme tests (polly#141)

A new `@fairfox/polly/test/contrast` export ships the WCAG-defined answer to
"does this pass contrast", so theme tests stop re-deriving it (and stop falling
into the channel-average and transparent-background traps). It provides
`relativeLuminance`, `contrastRatio`, `parseColor` (hex, `rgb()/rgba()`,
`hsl()/hsla()`, `transparent`, named colors), `compositeOver`,
`effectiveBackground` (walks the ancestor chain past transparent layers), and
`assertContrast(element, { min, against })` with a readable failure message. The
pure functions are transport-agnostic (Node and `page.evaluate` alike); the DOM
helpers read `getComputedStyle`.

### Fixed

#### Regression test for the dropdown reopen fix (polly#140)

The close-then-reopen bug on the last `[data-overlay-id]` popover was fixed in
0.75.3 but had no regression test. One now guards both halves of that fix —
`closeTopOverlay()` only acting on open overlays, and the `mousedown` handler
ignoring `button[popovertarget]` invokers — so the fix can't silently regress.
