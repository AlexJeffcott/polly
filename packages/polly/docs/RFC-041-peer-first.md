...
**History growth.** Automerge op history grows monotonically. Compaction (`Automerge.save`) produces a snapshot and drops history, at the cost of losing the causal record. The long-term aim is for Polly to ship a compaction operation with the preconditions described above and a configurable policy — compact documents older than N days, or compact when history exceeds M operations — so that applications do not implement their own.

That aim has not yet shipped (as of 0.67.0). What Polly does ship today is the integration seam: `registerDocIdResolver` and `registerRedirectDetector` (ADR 0008) let an application redirect a logical key from a bloated historical doc to a freshly-seeded successor without any wrapper code knowing it happened, and `CrdtPrimitive.changeCount` exposes the underlying Automerge change count so a consumer can decide when to compact. The compaction policy itself — which docs are eligible, what the threshold is, whether to snapshot the full state or filter tombstones, who signs the redirect — lives in the consumer for now. Polly issue #115 tracks the open question of which parts of this belong inside the framework: heartbeat-style auto-compaction, sealed-doc storage-GC, and per-wrapper policy hints are the live candidates.
