...
describe("Context Inference", () => {
  ...
  test("WebSocket handlers are tagged with Server context", async () => {
    ...
    const serverHandlers = analysis.handlers.filter((h) => h.location.file.includes("server.ts"));
  ...
  test("Electron handlers are tagged with MainProcess context", async () => {
    ...
    const mainHandlers = analysis.handlers.filter((h) => h.location.file.includes("main.ts"));
