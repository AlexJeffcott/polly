...
export class WebSocketAdapter implements RoutingAdapter<WebSocketAdapterConfig> {
  ...
  extractModel(): CoreVerificationModel {
    ...
    const messageTypes: MessageType[] = Array.from(messageTypeNames).map((name) => ({
      ...
      routing: {
        from: name.startsWith("USER_") || name.startsWith("CHAT_") || name.startsWith("TODO_") || name.startsWith("SYNC_")
          ? ["client-*"] // Client messages
          : ["server"], // Server messages
        to: name.startsWith("USER_") || name.startsWith("CHAT_") || name.startsWith("TODO_") || name.startsWith("SYNC_")
          ? ["server"] // To server
          : ["client-*"], // To clients
      },
    }))
    ...
    const routingRules: RoutingRule[] = [
      {
        ...
        description: "WebSocket hub-and-spoke: clients send to server, server broadcasts to clients",
      },
    ]
