...
async function generateCommand() {
	...
	try {
		...
		if (hasManifest) {
			...
			console.log(color("   Type: Generic TypeScript Project", COLORS.gray));
			console.log(
				color(
					"   ℹ No manifest.json found, using tsconfig-based detection",
					COLORS.gray,
				),
			);
		}
		...
		console.log(
			color(
				`\n✓ Found ${Object.keys(analysis.contexts).length} context(s)`,
				COLORS.green,
			),
		);
		console.log(
			color(
				`✓ Found ${analysis.messageFlows.length} message flow(s)`,
				COLORS.green,
			),
		);
		console.log(
			color(
				`✓ Found ${analysis.integrations.length} integration(s)`,
				COLORS.green,
			),
		);
		...
		for (const [contextType, contextInfo] of Object.entries(
			...
			console.log(
				color(`      - ${contextInfo.handlers.length} handler(s)`, COLORS.gray),
			);
			console.log(
				color(
					`      - ${contextInfo.chromeAPIs.length} Chrome API(s)`,
					COLORS.gray,
				),
			);
			if (contextInfo.components) {
				console.log(
					color(
						`      - ${contextInfo.components.length} component(s)`,
						COLORS.gray,
					),
				);
			}
		...
		if (analysis.integrations.length > 0) {
			...
			for (const integration of analysis.integrations) {
				console.log(
					color(`    • ${integration.name} (${integration.type})`, COLORS.gray),
				);
			}
		}
		...
		console.log(
			color("✅ Architecture documentation generated!\n", COLORS.green),
		);
		...
		console.log(
			color(`     -v ${outputDir}:/usr/local/structurizr \\`, COLORS.gray),
		);
		...
		console.log(
			color("   2. Create a workspace and get API credentials", COLORS.gray),
		);
		console.log(
			color(
				"   3. docker run -it --rm -v $(pwd)/docs:/usr/local/structurizr \\",
				COLORS.gray,
			),
		);
		console.log(
			color(
				"        structurizr/cli push -id WORKSPACE_ID -key KEY -secret SECRET \\",
				COLORS.gray,
			),
		);
		console.log(
			color(
				"        -workspace /usr/local/structurizr/architecture.dsl",
				COLORS.gray,
			),
		);
		console.log();
...
async function serveCommand(args: string[]) {
	...
	try {
		...
		if (!fs.existsSync(indexPath)) {
			...
			console.error(
				"   Run 'bun visualize --export' first to generate the site",
			);
			process.exit(1);
		}
...
function showHelp() {
	console.log(`
${color("bun visualize", COLORS.blue)} - Architecture visualization for TypeScript projects
...
${color("Supported Project Types:", COLORS.blue)}
...
${color("What gets generated:", COLORS.blue)}
  ...
  • Container diagram - Application contexts (server, client, worker, etc.)
