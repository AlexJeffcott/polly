...
export class StructurizrDSLGenerator {
  ...
  private getComponentTags(messageType: string, handler: any): string[] {
    ...
    } else if (type.includes('add') || type.includes('create') || type.includes('update') ||
               type.includes('delete') || type.includes('remove') || type.includes('toggle')) {
      tags.push('CRUD')
  ...
  private generateComponentRelationships(contextType: string, contextInfo: any): string {
    ...
    for (const [messageType, handlers] of handlersByType) {
      ...
      if (type.includes('add') || type.includes('create') || type.includes('update') ||
          type.includes('delete') || type.includes('remove') || type.includes('toggle') ||
          type.includes('clear') || type.includes('login') || type.includes('logout')) {
