  ...
  if (options.sync) {
    ...
    if (messageBus) {
      ...
      messageBus.on("STATE_SYNC", async (payload) => {
        ...
        // Only accept strictly newer clock values
        if (payload.clock > entry.clock) {
          ...
          if (deepEqual(entry.signal.value, payload.value)) {
            // Value hasn't changed, but update clock to stay in sync
        ...
        // Same or older clock: ignore (storage will resolve conflicts)
