...
test("Integration - Content to Background communication", async () => {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: Test handler with flexible return type
  const handler = mock(async () => ({ settings: { debugMode: true } })) as any;
...
test("Integration - Settings synchronization across contexts", async () => {
  ...
  expect(response.settings).toEqual({ debugMode: true, theme: "dark" } as any);
});
