...
test("Scenario: User changes settings - full log trail", async () => {
  ...
  const settingsLogs = logsResponse.logs.filter(
    (log: unknown) =>
      // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from LogStore
      (log as any).message.includes("settings") || (log as any).message.includes("Settings")
  );
...
test("Scenario: Debug production issue using logs", async () => {
  ...
  const sessionLogs = allLogsResponse.logs.filter(
    // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from LogStore
    (log: unknown) => (log as any).context?.sessionId === sessionId
  );
  ...
  // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from LogStore
  const errorLog = sessionLogs.find((log: unknown) => (log as any).level === "error");
...
test("Scenario: Investigate slow operation with logs", async () => {
  ...
  const opLogs = logsResponse.logs.filter(
    // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from LogStore
    (log: unknown) => (log as any).context?.operationId === operationId
  );
  ...
  // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from LogStore
  const slowStep = opLogs.find((log: unknown) => (log as any).context?.step === "slow-step");
...
test("Scenario: Operation fails and recovers with retry", async () => {
  ...
  const requestLogs = logsResponse.logs.filter(
    // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from LogStore
    (log: unknown) => (log as any).context?.requestId === requestId
  );
...
test("Scenario: User exports logs for bug report", async () => {
  ...
  // Verify structure
  // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from JSON
...
test("Scenario: Trace message flow across contexts", async () => {
  ...
  const messageLogs = logsResponse.logs.filter(
    // biome-ignore lint/suspicious/noExplicitAny: Log entries have dynamic structure from LogStore
    (log: unknown) => (log as any).context?.messageId === messageId
  );
