...
test("Invalid message payload does NOT cause loop", async () => {
  ...
  // Route message with empty payload (edge case)
  try {
    await router.routeMessage({
      id: "test-msg",
      source: "background",
      ...
      tabId: 123,
      timestamp: Date.now(),
      // biome-ignore lint/suspicious/noExplicitAny: Testing edge case with empty payload
...
test("Routing with missing required fields does NOT loop", async () => {
  ...
  const result = await router.routeMessage({
    ...
    source: "background",
    target: "content",
    // biome-ignore lint/suspicious/noExplicitAny: Testing edge case with missing required field
    tabId: undefined as any, // Missing required field
    timestamp: Date.now(),
    payload: { type: "TEST" },
  });
