...
import type { CodebaseAnalysis, StateAssignment, VerificationConfig } from "../../types";
...
describe("Compound Assignments Detection", () => {
  ...
  const createAnalysis = (assignments: StateAssignment[] = []): CodebaseAnalysis => ({
    stateType: null,
    messageTypes: ["test"],
    fields: [],
    handlers: [
  ...
  describe("Addition Assignment (+=)", () => {
    test("translates count += 1 to @ + 1", async () => {
      ...
      const analysis = createAnalysis([{ field: "count", value: "@ + 1" }]);
    ...
    test("translates total += amount", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "total", value: "@ + amount" }]);
    ...
    test("translates count += 5 (literal)", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "count", value: "@ + 5" }]);
    ...
    test("translates += with complex expression", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "count", value: "@ + (x * 2)" }]);
    ...
    test("translates += with variable reference", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "total", value: "@ + delta" }]);
    ...
    test("translates += 0 (no-op but valid)", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "count", value: "@ + 0" }]);
    ...
    test("handles += with parentheses", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "count", value: "@ + (value)" }]);
  ...
  describe("Subtraction Assignment (-=)", () => {
    ...
    test("handles -= with variable reference", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "total", value: "@ - cost" }]);
  ...
  describe("Multiplication, Division, Modulo", () => {
    test("translates count *= 2", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "count", value: "@ * 2" }]);
    ...
    test("translates total *= factor", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "total", value: "@ * factor" }]);
    ...
    test("translates total /= divisor", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "total", value: "@ / divisor" }]);
    ...
    test("translates count %= 10", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "count", value: "@ % 10" }]);
    ...
    test("translates value %= modulus", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "value", value: "@ % modulus" }]);
    ...
    test("handles *= with complex expression", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "count", value: "@ * (x + 1)" }]);
    ...
    test("handles /= with float", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "total", value: "@ / 2.5" }]);
  ...
  describe("Array Push Operations", () => {
    test("translates items.push(item) to Append", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Append(@, item)" }]);
    ...
    test("translates items.push(value) with literal", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Append(@, 42)" }]);
    ...
    test("translates items.push(string)", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: 'Append(@, "value")' }]);
    ...
    test("translates push with object", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Append(@, obj)" }]);
    ...
    test("translates push with variable reference", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Append(@, newItem)" }]);
    ...
    test("handles push with function result", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Append(@, getItem())" }]);
    ...
    test("translates push with property access", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Append(@, data.value)" }]);
    ...
    test("generates valid spec with push", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Append(@, x)" }]);
  ...
  describe("Array Pop, Shift, Unshift Operations", () => {
    test("translates items.pop() to SubSeq", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "SubSeq(@, 1, Len(@)-1)" }]);
    ...
    test("translates items.shift() to Tail", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "Tail(@)" }]);
    ...
    test("translates items.unshift(item) to prepend", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "<<item>> \\o @" }]);
    ...
    test("translates unshift with literal", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: "<<42>> \\o @" }]);
    ...
    test("translates unshift with string", async () => {
      const config = createConfig();
      const analysis = createAnalysis([{ field: "items", value: '<<"first">> \\o @' }]);
