...
describe("Enhanced DSL Generation - REAL Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (!fs.existsSync(tsConfigPath)) {
      throw new Error(`Test fixture not found: ${tsConfigPath}. Run setup first.`);
    }
  ...
  describe("Project Analysis", () => {
    ...
    test("should detect ws integration", () => {
      const wsIntegration = analysis.integrations.find((i) => i.name === "ws");
  ...
  describe("Automatic Relationship Detection", () => {
    test("should detect function call relationships from handler code", () => {
      ...
      const queryHandler = serverContext?.handlers.find((h) => h.messageType === "query");
    ...
    test("should detect multiple relationships from command handler", () => {
      const serverContext = analysis.contexts["server"]!;
      const commandHandler = serverContext?.handlers.find((h) => h.messageType === "command");
      ...
      expect(userServiceRelationship).toBeDefined();
      expect(userServiceRelationship?.description).toContain("executeUserCommand");
    });

    test("should detect relationships from auth handler", () => {
      const serverContext = analysis.contexts["server"]!;
      const authHandler = serverContext?.handlers.find((h) => h.messageType === "auth");
  ...
  describe("DSL Output with Automatic Relationships", () => {
    ...
    test("should include auto-detected relationships in DSL", () => {
      ...
      expect(dsl).toMatch(
        /query_handler\s*->\s*user_service\s+"Calls listUsers\(\)"\s+"Function Call"/
      );

      // Should include command handler -> user_service relationship
      expect(dsl).toMatch(
        /command_handler\s*->\s*user_service\s+"Calls executeUserCommand\(\)"\s+"Function Call"/
      );

      // Should include auth handler -> auth_service relationship
      expect(dsl).toMatch(
        /auth_handler\s*->\s*auth_service\s+"Calls authenticate\(\)"\s+"Function Call"/
      );
  ...
  describe("Default Styling", () => {
    ...
    test("should include theme URL", () => {
      ...
      expect(dsl).toContain("theme https://static.structurizr.com/themes/default/theme.json");
    });
  ...
  describe("Properties & Metadata", () => {
    ...
    test("should include message type and pattern", () => {
      ...
      expect(dsl).toMatch(
        /auth_handler[^}]*properties\s*\{[^}]*"Message Type"\s+"Authentication"/s
      );
      expect(dsl).toMatch(
        /auth_handler[^}]*properties\s*\{[^}]*"Pattern"\s+"Authentication Handler"/s
      );
    });
    ...
    test("should allow custom properties", () => {
      ...
      expect(dsl).toMatch(
        /query_handler[^}]*properties\s*\{[^}]*"Custom Property"\s+"Custom Value"/s
      );
  ...
  describe("Component Relationships", () => {
    test("should support explicit relationships between components", () => {
      ...
      expect(dsl).toMatch(
        /query_handler\s*->\s*user_service\s+"Calls listUsers\(\)"\s+"TypeScript"/
      );
      expect(dsl).toMatch(
        /command_handler\s*->\s*user_service\s+"Calls executeUserCommand\(\)"\s+"TypeScript"/
      );
  ...
  describe("Dynamic Diagrams", () => {
    test("should generate user-provided dynamic diagrams", () => {
            ...
            steps: [
              {
                ...
                from: "user",
                to: "extension.server",
                description: "Sends WebSocket message",
              },
              {
                order: 2,
                from: "extension.server",
                to: "extension.server.query_handler",
                description: "Routes to query handler",
              },
              {
                order: 3,
                from: "extension.server.query_handler",
                to: "extension.server",
      ...
      expect(dsl).toContain(
        'extension.server -> extension.server.query_handler "Routes to query handler"'
      );
      expect(dsl).toContain(
        'extension.server.query_handler -> extension.server "Returns query result"'
      );
    });
            ...
            steps: [
              {
                order: 1,
                from: "user",
                to: "extension.server.command_handler",
  ...
  describe("Perspectives", () => {
    ...
    test("should handle components without perspectives", () => {
      const dsl = generateStructurizrDSL(analysis, {
        componentDiagramContexts: Object.keys(analysis.contexts),
        perspectives: {
          query_handler: [{ name: "Security", description: "Low risk" }],
        },
      });
        ...
        deploymentNodes: [
          {
            ...
            containerInstances: [{ container: "server", instances: 2 }],
          },
        ],
      });
...
describe("Cross-File Relationship Detection (Lingua Architecture)", () => {
  ...
  beforeAll(async () => {
    ...
    const totalRelationships = Object.values(analysis.contexts).reduce(
      (sum, ctx) =>
        sum +
        ctx.handlers.reduce((handlerSum, h) => handlerSum + (h.relationships?.length || 0), 0),
      0
    );
  ...
  test("should detect userService.listUsers() relationship", () => {
    ...
    const userServiceRel = queryHandler?.relationships?.find((r) => r.to === "user_service");
  ...
  test("should detect repositories relationship in command handler", () => {
    ...
    const repositoriesRel = commandHandler?.relationships?.find((r) => r.to === "repositories");
  ...
  test("should generate complete DSL with cross-file relationships", () => {
    ...
    console.log(`   Found ${autoDetectedMatches?.length} auto-detected relationships`);
  });
});
