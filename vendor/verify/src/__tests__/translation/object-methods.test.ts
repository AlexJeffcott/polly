...
describe("Object/Array Method Translation", () => {
	...
	describe("Array Includes", () => {
		...
		test("translates includes in conjunction", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.includes('a') && state.users.includes('b')", location: { line: 1, column: 1 } },
			];
	...
	describe("Array .some() Method", () => {
		...
		test("translates some with comparison", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.some(item => item.id === 'target')", location: { line: 1, column: 1 } },
			];
		...
		test("translates some with complex condition", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.some(i => i.count > 0 && i.active)", location: { line: 1, column: 1 } },
			];
	...
	describe("Array .every() Method", () => {
		...
		test("translates every with comparison", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.every(item => item.count > 0)", location: { line: 1, column: 1 } },
			];
		...
		test("translates every with parenthesized parameter", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.users.every((user) => user.active)", location: { line: 1, column: 1 } },
			];
		...
		test("translates every with complex condition", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.every(i => i.count >= 0 && i.valid)", location: { line: 1, column: 1 } },
			];
		...
		test("translates every in conjunction", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.every(i => i.active) && state.count > 0", location: { line: 1, column: 1 } },
			];
	...
	describe("Array .find() Method", () => {
		test("translates simple find", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.find(i => i.id === 'target') !== null", location: { line: 1, column: 1 } },
			];
		...
		test("translates find with comparison", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.find(item => item.count > 5)", location: { line: 1, column: 1 } },
			];
		...
		test("translates find with parenthesized parameter", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.users.find((user) => user.name === 'alice')", location: { line: 1, column: 1 } },
			];
		...
		test("translates find with complex condition", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.find(i => i.active && i.count > 0)", location: { line: 1, column: 1 } },
			];
		...
		test("translates find in property access", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.find(i => i.id === payload.targetId).value", location: { line: 1, column: 1 } },
			];
	...
	describe("Array .filter() Method", () => {
		test("translates filter().length", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.filter(i => i.active).length > 0", location: { line: 1, column: 1 } },
			];
		...
		test("translates filter().length with comparison", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.filter(item => item.count > 0).length", location: { line: 1, column: 1 } },
			];
		...
		test("translates filter().length with parenthesized parameter", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.users.filter((user) => user.active).length === 3", location: { line: 1, column: 1 } },
			];
		...
		test("translates filter().length with complex condition", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.filter(i => i.count > 0 && i.valid).length >= 1", location: { line: 1, column: 1 } },
			];
		...
		test("translates filter().length in comparison", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.filter(i => i.active).length < state.count", location: { line: 1, column: 1 } },
			];

			const { spec } = await generator.generate(baseConfig, baseAnalysis);

			expect(spec).toContain("Cardinality({i \\in contextStates[ctx].items : i.active}) < contextStates[ctx].count");
		});
	});
	...
	describe("String Methods", () => {
		...
		test("translates endsWith", async () => {
			...
			expect(spec).toContain("SubSeq(contextStates[ctx].name, Len(contextStates[ctx].name) - Len('ce') + 1, Len(contextStates[ctx].name)) = 'ce'");
		});
		...
		test("translates startsWith with variable", async () => {
			...
			expect(spec).toContain("SubSeq(contextStates[ctx].name, 1, Len(payload.prefix)) = payload.prefix");
		});

		test("translates multiple string methods", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.name.startsWith('a') && state.name.endsWith('e')", location: { line: 1, column: 1 } },
			];
		...
		test("translates slice with start index", async () => {
			...
			expect(spec).toContain("SubSeq(contextStates[ctx].name, 2, Len(contextStates[ctx].name)) = 'lice'");
		});
		...
		test("translates slice with negative index", async () => {
			...
			expect(spec).toContain("SubSeq(contextStates[ctx].name, Len(contextStates[ctx].name) - 2 + 1, Len(contextStates[ctx].name)) = 'ce'");
		});
	...
	describe("Combined Operations", () => {
		test("combines length and includes", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.length > 0 && state.items.includes('test')", location: { line: 1, column: 1 } },
			];
		...
		test("combines some and every", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.some(i => i.active) && state.users.every(u => u.valid)", location: { line: 1, column: 1 } },
			];
		...
		test("combines filter and find", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.filter(i => i.active).length > 0 || state.items.find(i => i.id === 'x')", location: { line: 1, column: 1 } },
			];
		...
		test("combines array indexing with methods", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items[0].active && state.items.some(i => i.valid)", location: { line: 1, column: 1 } },
			];
		...
		test("combines string and array methods", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.name.startsWith('a') && state.items.length > 0", location: { line: 1, column: 1 } },
			];
	...
	describe("Edge Cases", () => {
		...
		test("handles multiple array references", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.length === state.users.length", location: { line: 1, column: 1 } },
			];
		...
		test("handles complex lambda expressions", async () => {
			baseAnalysis.handlers[0]!.preconditions = [
				{ expression: "state.items.some(i => i.count > state.count && i.active)", location: { line: 1, column: 1 } },
			];
