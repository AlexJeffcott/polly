...
export class TLAValidator {
	...
	validateBrackets(text: string): ValidationError | null {
		...
		// Track /\ and \/ balance
		let conjunctionBalance = 0;
		let disjunctionBalance = 0;
			...
			if (Object.values(pairs).includes(char)) {
				...
				if (!top || pairs[top.char] !== char) {
					return {
						...
						suggestion: top ? `Expected '${pairs[top.char]}' to match '${top.char}' at position ${top.pos}` : undefined,
					};
				}
			}
		}
	 ...
	 */
	validateOperator(op: string, context: "action" | "invariant" | "temporal"): ValidationError | null {
		// Priming only in actions
