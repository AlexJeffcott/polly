...
export class SANYRunner {
	...
	private parseSANYOutput(result: DockerRunResult, specPath: string): ValidationResult {
		...
		if (result.exitCode === 0 && !output.includes("*** Errors:") && !output.includes("***Parse Error***") && !output.includes("Fatal errors") && !output.includes("*** Warnings:")) {
			return {
				valid: true,
				errors: [],
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (lexicalMatch && lexicalMatch[1] && lexicalMatch[2] && lexicalMatch[3]) {
				errors.push({
					type: "lexical",
			...
			if (parseMatch && parseMatch[1] && parseMatch[2] && parseMatch[3]) {
				errors.push({
					type: "syntax",
			...
			const semanticLocationMatch = line.match(/^line (\d+), col (\d+) to line \d+, col \d+ of module (\w+)/);
			if (semanticLocationMatch && semanticLocationMatch[1] && semanticLocationMatch[2]) {
				// Collect message lines (until we hit another "line" or empty line or end)
				const msgLines = [];
				let j = i + 1;
				while (j < lines.length) {
					const msgLine = lines[j];
					if (!msgLine || msgLine.trim() === "" || msgLine.startsWith("line ") || msgLine.includes("***")) {
						break;
					}
				...
				if (inWarningsSection) {
					warnings.push(`line ${semanticLocationMatch[1]}, col ${semanticLocationMatch[2]}: ${message || "Semantic warning"}`);
				} else {
					errors.push({
						type: "semantic",
			...
			if (semanticMatch && semanticMatch[1] && semanticMatch[2] && semanticMatch[3]) {
				errors.push({
					type: "semantic",
